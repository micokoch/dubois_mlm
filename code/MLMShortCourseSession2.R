...
lastest=glmmLasso(fix=tumorsize ~ Sex + Age+ FamilyHx + LengthofStay + RBC ,  rnd=list(DID=~1), lambda=50, data=hdp)
summary(lastest)
install.packages("LMERConvenienceFunctions")
library(LMERConvenienceFunctions)
ffRanefLMER.fnc(model=l1pred,ran.effects=list(slopes=c("Sex", "Age", "FamilyHx", "LengthofStay"),by.vars="DID", corr=c(1,1,1,1)), if.warn.not.add=FALSE, alpha=0.05)
l1pred2=lmer(tumorsize ~ Sex + Age+ FamilyHx + LengthofStay + (1+LengthofStay|DID), data=hdp)
test=allFit(l2predrs)
summary(test)
l1pred2=lmer(tumorsize ~ Sex + Age+ FamilyHx + LengthofStayc + (1+LengthofStayc|DID), data=hdp)
summary(l1pred2)
l2pred=lmer(tumorsize ~ Sex + Age+ FamilyHx + LengthofStayc + Experience + School + 
...
summary(l2pred)
step(l2pred)
l2predrsnol2=lmer(tumorsize ~ Sex + Age+ FamilyHx + LengthofStay + (1+LengthofStay|DID),REML=FALSE, data=hdp, control =
lmerControl(optimizer = "bobyqa"))
anova(l2predrs,l2predrsnol2)
library(redres)
shapiro.test(hdp$raw)
launch_redres(l2predrs)
